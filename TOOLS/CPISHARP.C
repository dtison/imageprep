...
char *argv[];
{
   ...
   if (argc < 3)
   {   
       fputs ("\nusage:  cpish <filename>.cpi  <filename>.cpi  <sharpness 1..16>\n\n",stdout); 
       exit (0);
   }
...
WORD  wBytesPerRow;
{
    ...
    /*  For each pixel, look at 8 surrounding and exaggerate the difference  
        We are actually processing (lpSource + 1)  */
...
static void dosharpen(U8 *dp)
{
	...
	for (x=0; x < new->width; ++x) {
		orig = *sp++;
		delta = (orig << 3) - line[2][x-1] - line[2][x+1];
		delta -= (line[1][x-1] + line[1][x] + line[1][x+1]);
		delta -= (line[3][x-1] + line[3][x] + line[3][x+1]);
